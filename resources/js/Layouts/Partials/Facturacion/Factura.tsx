...
import { Search, Eye, Ban, Calendar, User, FileText, BanknoteIcon } from "lucide-react";
...
const Factura = ({ comprobantes = [] }: FacturaProps) => {
    const [searchTerm, setSearchTerm] = useState("");
    const [isConfirmOpen, setIsConfirmOpen] = useState(false);
    const [selectedId, setSelectedId] = useState<number | null>(null);

    const filteredComprobantes = comprobantes.filter(
        comp => comp.tipo_comprobante.toLowerCase() === 'factura' &&
        ...
        <div className="space-y-4">
            <Card>
                <CardHeader className="pb-4">
                    <CardTitle className="text-lg md:text-xl">Facturas</CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    {filteredComprobantes.length === 0 && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-500 text-sm">
                                {searchTerm ? 'No se encontraron facturas que coincidan con tu búsqueda.' : 'No hay facturas disponibles.'}
                            </p>
                        </div>
                    )}
